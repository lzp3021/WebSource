...
public class ShiroDefinitionSectionMetaSource implements FactoryBean<Ini.Section> {
	...
	public Section getObject() throws Exception {
		...
		for(SecResource resource : resources) {
			...
			for (SecAuthority authority : resource.getAuthorities()) {
				...
				if(resource.getResourceContent().indexOf(";") != -1) {
					...
					for(String source : sources) {
					    putDefinitionSection(section, source, authority.getAuthorityCode());
					}
				} else {
				    putDefinitionSection(section, resource.getResourceContent(), authority.getAuthorityCode());
				}
			}
		}
        section.put("/**", "user");
